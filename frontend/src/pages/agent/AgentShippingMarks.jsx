...
import { FaMapMarkerAlt, FaPlus, FaEdit, FaCopy, FaCheck } from "react-icons/fa";
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isCorporateAgent && !editAddress && displayAddress) {
        ...
        if (!userMarkId) {
          toast.error("Shipping mark ID not found. Please ensure you have a shipping mark.");
          setIsLoading(false);
          return;
        }
        ...
        const baseAddress = displayAddress.fullAddress || displayAddress.full_address || "";
        ...
        const originalShippingMark = editAddress.shipping_mark || editAddress.shippingMark || "";
        ...
        payload = {
          ...
          trackingNumber: editAddress.tracking_number || editAddress.trackingNumber || "",
        };
      } else {
      ...
      if (editAddress) {
        ...
        toast.success(isCorporateAgent ? "Agent address generated successfully!" : "Shipping address added successfully");
      }
        ...
        <button
          onClick={() => {
            if (currentUser?.agent_type === "corporate" && !displayAddress) {
              toast.error("Base address not set. Please contact admin to set the base address first.");
              return;
            }
            setShowAddForm(true);
            setEditAddress(null);
          ...
          disabled={currentUser?.agent_type === "corporate" && addresses.length > 0}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg ${
            currentUser?.agent_type === "corporate" && addresses.length > 0
              ? "bg-gray-400 text-gray-200 cursor-not-allowed"
              : "bg-purple-600 text-white hover:bg-purple-700"
          }`}
          title={currentUser?.agent_type === "corporate" && addresses.length > 0 ? "You can only generate one address. Edit your existing address instead." : ""}
        >
          <FaPlus /> {currentUser?.agent_type === "corporate" ? "Generate Agent Address" : "Add Address"}
        </button>
      </div>
      ...
      {showAddForm && (
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            {currentUser?.agent_type === "corporate" && !editAddress && displayAddress ? (
              ...
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    ...
                    {!userMarkId && (
                      <span className="ml-2 text-red-500 text-xs">⚠️ Not detected</span>
                    )}
                  </label>
                  <input
                    type="text"
                    value={userMarkId || "Not found - Please ensure you have a shipping mark"}
                    disabled
                ...
                <div>
                  ...
                  <textarea
                    rows="8"
                    value={displayAddress && userMarkId && businessName ? (() => {
                      const baseAddress = displayAddress.fullAddress || displayAddress.full_address || "";
                      const phoneNumber = currentUser?.contact || currentUser?.phone || "";
                      const country = "加纳";
                      const shippingMark = `${userMarkId}:${businessName.trim()}`;
                  ...
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    This address will be generated from the base address set by admin
                  </p>
                </div>
              </>
              ...
              <>
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Only the business name can be edited. The address will be regenerated with the new name.
                  </p>
                </div>
