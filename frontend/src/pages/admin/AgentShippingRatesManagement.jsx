...
import { FaSave, FaHistory, FaDollarSign, FaSpinner } from "react-icons/fa";
...
const AgentShippingRatesManagement = () => {
  ...
  const fetchCurrentRate = async () => {
    try {
      ...
      if (error.response?.status !== 404) {
        ...
        toast.error("Failed to load agent shipping rates");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveContainerRate = async (e) => {
    ...
    try {
      ...
      toast.success("Container agent rate saved");
  ...
  const handleClearContainerRate = async (container) => {
    ...
    if (!window.confirm("Remove agent rate for this container? It will use the global agent rate.")) return;
    try {
      await API.delete("/buysellapi/container-shipping-rates/", {
        params: { container_id: id, is_agent_rate: "true" },
      });
      toast.success("Container agent rate cleared");
      const ratesResp = await API.get("/buysellapi/container-shipping-rates/");
      setContainerRates(Array.isArray(ratesResp.data) ? ratesResp.data : []);
    } catch (err) {
  ...
  if (loading && !rates.normal_goods_rate) {
    return (
      <div className="p-6 flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <FaSpinner className="animate-spin text-4xl text-blue-600 mx-auto mb-4" />
          <p className="text-gray-600 dark:text-gray-400">Loading agent shipping rates...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Set and manage shipping rates per CBM for agents. These rates are separate from regular customer shipping rates.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">
            Global Agent Shipping Rate (default for all containers)
          </h3>
          <button
      ...
      {/* Per-container agent rates */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">
          Per-container agent rates
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Set an agent shipping rate for a specific container. Agent trackings and invoices for that container will use this rate instead of the global agent rate.
        </p>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-gray-700 dark:text-gray-200">Agent rate</th>
                  ...
                  return (
                    <tr key={containerId}>
                      ...
                      <td className="px-4 py-2 text-right">
                        <button type="button" onClick={() => openContainerRateModal(c, existing)} className="text-pink-600 dark:text-pink-400 hover:underline mr-2">{existing ? "Edit" : "Set rate"}</button>
                        {existing && <button type="button" onClick={() => handleClearContainerRate(c)} className="text-gray-500 dark:text-gray-400 hover:underline">Clear</button>}
                      </td>
                    </tr>
                  );
                })
      ...
      {showContainerModal && selectedContainer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Agent rate for {selectedContainer.container_number ?? selectedContainer.id ?? "container"}</h3>
            <form onSubmit={handleSaveContainerRate}>
              <div className="space-y-4">
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Normal (per CBM) $</label><input type="number" step="0.01" min="0" required value={containerRateForm.normal_goods_rate} onChange={(e) => setContainerRateForm((f) => ({ ...f, normal_goods_rate: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white" /></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Special (per CBM) $</label><input type="number" step="0.01" min="0" required value={containerRateForm.special_goods_rate} onChange={(e) => setContainerRateForm((f) => ({ ...f, special_goods_rate: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white" /></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Normal (CBM &lt; 1) $</label><input type="number" step="0.01" min="0" required value={containerRateForm.normal_goods_rate_lt1} onChange={(e) => setContainerRateForm((f) => ({ ...f, normal_goods_rate_lt1: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white" /></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Special (CBM &lt; 1) $</label><input type="number" step="0.01" min="0" required value={containerRateForm.special_goods_rate_lt1} onChange={(e) => setContainerRateForm((f) => ({ ...f, special_goods_rate_lt1: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white" /></div>
              </div>
